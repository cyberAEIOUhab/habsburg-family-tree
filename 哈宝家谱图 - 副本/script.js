  ...
  function showNodeDetails(nodeData) {
      // 构建详细信息
      let details = `
          <h3 style="color: #f5e6d3; margin-bottom: 1.5rem; border-bottom: 2px solid #c19a6b;
  padding-bottom: 0.8rem; text-shadow: 1px 1px 2px rgba(0,0,0,0.3);">${nodeData.name}</h3>
      `;

      // 基本信息
      if (nodeData.birth && nodeData.death) {
          details += `<p style="color: #d4b896; margin-bottom: 0.8rem;"><strong style="color:
  #f5e6d3;">生卒:</strong> ${nodeData.birth} - ${nodeData.death}</p>`;
      }

      if (nodeData.reign) {
          details += `<p style="color: #d4b896; margin-bottom: 0.8rem;"><strong style="color:
  #f5e6d3;">统治时期:</strong> ${nodeData.reign}</p>`;
      }

      if (nodeData.title) {
          details += `<p style="color: #d4b896; margin-bottom: 1.5rem;"><strong style="color:
  #f5e6d3;">头衔:</strong> ${nodeData.title}</p>`;
      }
      ...
      if (nodeData.children && nodeData.children.length > 0) {
          details += `<p style="color: #d4b896; margin-top: 1.5rem;"><strong style="color:
  #f5e6d3;">子女数量:</strong> ${nodeData.children.length}</p>`;
      }

      // 详细描述
      if (nodeData.description) {
          details += `<div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid
  rgba(193, 154, 107, 0.3);">
      ...
      modalContent.style.cssText = `
          background:
              ...
              url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="120"
  height="120" viewBox="0 0 120 120"><defs><pattern id="modalPattern" x="0" y="0" width="120"
  height="120" patternUnits="userSpaceOnUse"><rect fill="%231e1814" width="120" height="120"/><path
  d="M0,0 L120,120 M120,0 L0,120" stroke="%23302822" stroke-width="1"
  opacity="0.15"/></pattern></defs><rect width="100%" height="100%"
  fill="url(%23modalPattern)"/></svg>');
          color: #f5e6d3;
          padding: 2.5rem;
          border-radius: 15px;
          max-width: 600px;
      ...
      modalContent.innerHTML = details + `
          <button onclick="this.parentElement.parentElement.remove()"
                  style="position: absolute; top: 1rem; right: 1rem; background: #8b4513; color:
  #f5e6d3; border: 1px solid #c19a6b; border-radius: 50%; width: 40px; height: 40px; cursor:
  pointer; font-size: 1.2rem; display: flex; align-items: center; justify-content: center;
